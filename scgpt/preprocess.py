...
class Preprocessor:
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        if self.binning:
            ...
            for row in layer_data:
                # 确保 row 是一个一维 NumPy 数组
                if issparse(row):
                    row = row.toarray().flatten()  # 转换为一维数组
                else:
                    row = np.asarray(row).flatten()  # 确保是一维 NumPy 数组
                
                # 创建一个独立的、一维的 NumPy 数组
                binned_row = np.zeros(row.shape[0], dtype=np.int64)  # 直接指定形状，避免视图影响
                
                # 检查是否全为零
                if row.max() == 0:
                    print("Warning: Row contains all zeros.")
                    binned_rows.append(binned_row)
                    bin_edges.append(np.array([0] * n_bins))
                    continue
                
                # 获取非零索引并处理
                non_zero_ids = np.nonzero(row)[0]  # 一维索引
                non_zero_row = row[non_zero_ids]
                bins = np.quantile(non_zero_row, np.linspace(0, 1, n_bins - 1))
                non_zero_digits = _digitize(non_zero_row, bins)
