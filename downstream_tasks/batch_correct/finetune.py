...
import scanpy as sc,numpy as np,pandas as pd,anndata as ad,pickle as pkl,scgpt as scg
import os,json,sys,time,torch,argparse,pickle
...
def our_step_preporcess(adata, species, human_mouse_align, vocab_temp):
    rna_data_pre = preprocss_rna(adata, species, human_mouse_align, vocab_temp)
    
    return rna_data_pre

def prepare_data(tokenized, species, mask_ratio, mask_value, pad_value, batch_ids=None):
...
def train(model, loader, device, vocab, pad_token, optimizer, scaler, config):
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config["amp"]):
            ...
            output_dict, transformer_out = model(
...
def evaluate(model, loader, device, vocab, pad_token, save_dir):
    ...
    with torch.no_grad():
        for batch, batch_data in enumerate(loader):
            ...
            rna_emb.append(output_dict["mlm_output"].cpu())
    rna_emb = torch.cat(rna_emb, dim=0)
    with open(os.path.join(save_dir, "rna_embeddings.pickle"), 'wb') as file:
        pickle.dump(rna_emb, file)
...
if __name__ == "__main__":
    ...
    encoded_batch = le.fit_transform(adata.obs['donor'].values)
    ...
    preprocessor(adata, batch_key="donor")
    ...
    if args.load_model:
        ...
        model_file = model_dir / "our_PBMC_model.pt" if args.mode == "train" else model_dir / "our_PBMC_model.pt"
    ...
    train_data_pt = prepare_data(tokenized_train, args.species, args.mask_ratio, -1, -2, batch_ids)
