...
def tokenize_and_pad_batch2(
    ...
    temp=data[:,-387:]
    data=data[:,:-387]
    temp_gene_ids=gene_ids[-387:]
    gene_ids=gene_ids[:-387]
    ...
    temp_tensor = torch.tensor(temp, dtype=batch_padded['values'].dtype)

    # 2. 使用 torch.cat() 沿列（维度1）拼接张量
    batch_padded['values'] = torch.cat((batch_padded['values'], temp_tensor), dim=1)
