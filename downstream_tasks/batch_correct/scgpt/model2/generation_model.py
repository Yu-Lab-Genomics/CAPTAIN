...
class TransformerGenerator(nn.Module):
    ...
    def pred_perturb(
        ...
        n_genes=5387
        batch_size=int(x.shape[0] / (2*n_genes))
        reshaped = x.view(batch_size, 2*n_genes)
        n_genes_2=5000
        ori_gene_values = reshaped[:, :n_genes_2]
        ori_adt_values = reshaped[:, n_genes_2:n_genes]
        pert_flags = reshaped[:, n_genes:(n_genes+n_genes_2)].long()
        ...
        if include_zero_gene in ["all", "batch-wise"]:
            ...
            # )
            src_key_padding_mask=(input_values==0)
            with torch.cuda.amp.autocast(enabled=amp):
                output_dict,transformerout = self(
        ...
        return pred_gene_values,transformerout,ori_adt_values,src_key_padding_mask
        # return output_dict,transformerout
