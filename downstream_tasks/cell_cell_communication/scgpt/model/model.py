...
class Human_RNA_Embedding(nn.Module):
    def __init__(self, weight_path):
        """
        Args:
            weight_path (str): .npy 文件的完整路径
        """
        super().__init__()
        

        if not os.path.exists(weight_path):
            raise FileNotFoundError(f"找不到文件: {weight_path}")
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        file_path = os.path.join(prior_know, 'final_human_prior_knwo.npy')
        self.human_species_embedding = Human_RNA_Embedding(file_path)
        file_path = os.path.join(prior_know, 'final_mouse_prior_knwo.npy')
        self.mouse_species_embedding = Mouse_RNA_Embedding(file_path)
        self.perturb_embedding=nn.Embedding(2,d_model)
        self.cross_attn_adt = None
        self.init_weights()
        
    def set_cross_attn_layer(self, layer: nn.Module):
        """
        用于从外部注入共享的跨注意力层 (nn.TransformerDecoderLayer)
        """
        self.cross_attn_adt = layer
    ...
    def _encode(
        ...
        if torch.all(species == 0):
            src_species = self.human_species_embedding(src)
        if torch.all(species == 1):
            src_species = self.mouse_species_embedding(src)
        ...
        if self.input_emb_style == "scaling":
            ...
            total_embs = src + values + src_species.to(torch.float32)
            # total_embs = src + values + src_species.to(torch.float32)+self.perturb_embedding(perturb.to(dtype=torch.long))
            # total_embs = src + values 
    ...
    def forward(
        ...
        # ====> 新增: 接收来自ADT模型的上下文
        adt_embed: Optional[torch.Tensor] = None,
        # adt_pad_mask: Optional[torch.Tensor] = None,
        # <==== 结束新增
    ) -> Mapping[str, Tensor]:
        """
        Args:
        ...
        transformer_output = self._encode(
            ...
            src, values, species,src_key_padding_mask, batch_labels
            # src, values, src_key_padding_mask, batch_labels
        )
        previous_transformer_output=transformer_output
        # 3. (RNA -> ADT) 如果注入了共享层和ADT上下文，则执行跨注意力
        if self.cross_attn_adt is not None and adt_embed is not None:
            # RNA (tgt) "查询" ADT (memory)
            transformer_output = self.cross_attn_adt(
                tgt=transformer_output,
                memory=adt_embed,
                tgt_key_padding_mask=src_key_padding_mask
                # memory_key_padding_mask=adt_pad_mask
            )
        # 4. 'memory' 现在是融合了ADT信息的RNA嵌入
        # <==== 结束改动
        ...
        return output,previous_transformer_output
    ...
    def encode_batch(
        ...
        species = torch.zeros_like(values)
        for i in trange(0, N, batch_size):
            raw_output = self._encode(
                src[i : i + batch_size].to(device),
                values[i : i + batch_size].to(device),
                species[i : i + batch_size].to(device),
