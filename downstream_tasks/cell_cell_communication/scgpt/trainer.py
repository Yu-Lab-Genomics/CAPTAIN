...
def prepare_data(
    ...
    # train_temp=tokenized_train['values'][:,-387:]
    # tokenized_train['values']=tokenized_train['values'][:,:-387]
    # valid_temp=tokenized_valid['values'][:,-387:]
    # tokenized_valid['values']=tokenized_valid['values'][:,:-387]
...
def train2(
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            output_dict,transformer_output = model.rna_model(
                input_gene_ids,
                input_values,
                src_key_padding_mask=src_key_padding_mask,
                batch_labels=batch_labels
            ...
            if config.DAR:
                ...
                loss = loss + config.dab_weight * loss_dab
                metrics_to_log.update({"train/dab": loss_dab.item()})
            loss = loss+ masked_mse_loss(adt_to_out.squeeze(-1), labels_adt_data, adt_mask)
            loss = loss+ quantile_loss(adt_to_out_quantiles, labels_adt_data, adt_mask)
        model.zero_grad()
        scaler.scale(loss).backward()
        scaler.unscale_(optimizer)
