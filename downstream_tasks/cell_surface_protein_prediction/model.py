...
class FastAttention(nn.Module):
    ...
    def forward(self, q, k, v, output_attentions = False):
        ...
        if output_attentions:
            ...
            attn_weights = torch.zeros(1, 1, q.shape[2], k.shape[2]).to(device).to(torch.float16)
...
class Performer(nn.Module):
    ...
    def forward(self, x, output_attentions, **kwargs):
...
class FeedForward(nn.Module): 
    def __init__(self, dim, ff_mult=4, dropout=0.1): 
        super().__init__()
        self.net = nn.Sequential( nn.Linear(dim, dim * ff_mult), nn.GELU(), nn.Dropout(dropout), nn.Linear(dim * ff_mult, dim), nn.Dropout(dropout) ) 
        self.norm = nn.LayerNorm(dim)
...
class PerformerLM_ADT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.token_emb = nn.Embedding(num_tokens, dim) #7 200
        ...
        self.to_out1 = nn.Sequential(
                                    nn.Linear(dim, 128),  # First hidden layer
                                    nn.ReLU(),            # Activation function (optional, you can change it as needed)
